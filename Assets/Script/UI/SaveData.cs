...
public class SaveData : MonoBehaviour
{
    ...
    [Serializable]
    public class CloudGameData : ICloneable
    {
        public object Clone()
        {
            CloudGameData newData = new CloudGameData();
            newData.userData = userData;
            newData.purchasedData = new List<ShopManager.ItemInfo>(purchasedData);
            //newData.purchasedData = this.purchasedData.ConvertAll(obj => (ShopManager.ItemInfo)obj.Clone());
            return newData;
        }
        public CloudGameData(CloudGameData data)
        {
            CloudGameData newData = new CloudGameData();
            newData.userData = userData;
            newData.purchasedData = new List<ShopManager.ItemInfo>(purchasedData);
    ...
    public void SavePurchasedShopData(List<ShopManager.ItemInfo> itemInfoList)
    {
        ...
        this.purchasedData = new List<ShopManager.ItemInfo>(itemInfoList);
        //saveData = data;
        file.Close();
    }
    ...
    public UserData GetLocalUserData()
    {
        if (userData == null)
        {
            //Debug.Log("Debug : Return null");
            return null;
        }
        return userData;
    }
    public List<ShopManager.ItemInfo> GetLocalPurchasedData()
    {
    ...
    public UserData GetCloudUserData()
    {
        if (userData == null)
        {
            //Debug.Log("Debug : Return null");
        ...
        return cloudGameData.userData;
    }

    public List<ShopManager.ItemInfo> GetCloudPurchasedData()
    {
        if (purchasedData == null)
        {
            //Debug.Log("Debug : Return null");
        ...
        return cloudGameData.purchasedData;
    }
    ...
    public bool SetCloudSaveDataToLocalData(CloudGameData cloudGameData, bool isApplyToLocal = true)
    {
        //Debug.Log("DEBUG : isApplyToLocal : " + isApplyToLocal);
        //Debug.Log("DEBUG : " + this.cloudGameData.userData.coins + " : " + this.cloudGameData.userData.bestScore);
        if (this.cloudGameData == null)
        {
            //Debug.Log("cloud data is null");
            if (cloudGameData.userData == null)
            {
                cloudGameData.userData = null;
            }
            if (cloudGameData.purchasedData == null)
            {
                cloudGameData.purchasedData = null;
            }
            return false;
        }
        ...
        //Debug.Log("DEBUG 2: " + this.cloudGameData.userData.coins + " : " + this.cloudGameData.userData.bestScore);
        if (isApplyToLocal == true)
        {
            ShopManager shopManager = FindObjectOfType<ShopManager>();

            //Debug.Log("DEBUG 2.5 : " + this.cloudGameData.userData.coins + " : " + this.cloudGameData.userData.bestScore);
            //GoogleCloudManager.Instance.PrintData(shopManager.itemInfoList);


            //infoOne = m.ConvertAll(obj => (ShopManager.ItemInfo)obj.Clone());

            userData = cloudGameData.userData;
            purchasedData = cloudGameData.purchasedData.ConvertAll(obj => (ShopManager.ItemInfo)obj.Clone());

            MainManager.Instance.SetDataFromCloudSave(cloudGameData);
            //List<ShopManager.ItemInfo> temp = new List<ShopManager.ItemInfo>(cloudGameData.purchasedData);
            shopManager.itemInfoList = cloudGameData.purchasedData.ConvertAll(obj => (ShopManager.ItemInfo)obj.Clone());
            //shopManager.itemInfoList = cloudGameData.purchasedData.ToList();
            shopManager.RefreshShopList();

            SaveUserData(userData.coins, userData.bestScore);

            //Debug.Log("DEBUG 3: " + this.cloudGameData.userData.coins + " : " + this.cloudGameData.userData.bestScore);
            //GoogleCloudManager.Instance.PrintData(shopManager.itemInfoList);
        }
        this.cloudGameData = (CloudGameData)cloudGameData.Clone();
        //this.cloudGameData = new CloudGameData(cloudGameData);
        return true;
    }
    ...
    public string GetCloudSaveData()
    {
        ...
        CloudGameData temp = new CloudGameData();
        temp = cloudGameData;

        var dat = JsonUtility.ToJson(temp);
        return dat;
    }
}
